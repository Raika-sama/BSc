...
export const useAssignedTests = (studentId) => {
    ...
    // Funzione di utilità per estrarre i test dalla risposta API
    ...
    const fetchAssignedTests = useCallback(async () => {
        ...
        try {
            ...
            const response = await axiosInstance.get(`/tests/assigned/student/${studentId}`, {
                params: {
                    include: 'assignedBy,attempts,configuration' // Richiediamo dati aggiuntivi
                }
            });
            ...
            console.debug('Processed tests array:', {
                ...
                firstTest: testsArray.length > 0 ? {
                    ...
                    assignedBy: testsArray[0].assignedBy,
                    attempts: testsArray[0].attempts,
                    configurazione: testsArray[0].configurazione
                } : 'No tests'
            });
            ...
            const activeSortedTests = sortedTests.filter(test => test.status !== 'completed');
